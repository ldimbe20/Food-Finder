...
export const FriendProfileList = () => {
    ...
    const getFriends = () => {  //this function is used to replace data information after ticket has been deleted
        return fetch("http://localhost:8088/friendFoodRestrictions?_expand=friends&_expand=foodRestrictions")
            .then(res => res.json())
            .then((data) => {
                setFriends(data)
            })
                ...
                friends.map(
                    (friend) => {
                        ...
                        return <div key={`friend--${friend.id}`}>
                                ...
                                <h4> {friend.friendFoodRestrictions.name} 
                                </h4>
                                
                                <h4> Doesn't Like: {friend.foodRestriction.name} 
                                </h4>
                                {/* <button onClick={() => {
                                    deleteRestriction(friend.foodRestriction.name)  // taking the argument of ticket.id and mapping through ticket array to find correct id number to delete
                                }}>Delete</button> */}

                                //!would I put a delet button here? would it be similar to line 25 
